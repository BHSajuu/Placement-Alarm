...
const TruncatedCell = ({ text, limit = 15 }: { text: string; limit?: number }) => {
  ...
  return (
    ...
    >
      <span className={`block transition-all duration-200 ${isExpanded ? "whitespace-normal" : "truncate"}`}>
        {isExpanded ? text : `${text.substring(0, limit)}...`}
      </span>
      {/* Tooltip hint on hover */}
      {!isExpanded && (
        <span className="invisible group-hover:visible absolute -top-8 left-0 z-50 bg-gray-950 text-yellow-500 text-xs px-2 py-1 rounded shadow-lg whitespace-nowrap">
          {text}
        </span>
      )}
    </div>
          ...
          <Table className="table-fixed w-full">
            <TableHeader className=" bg-gray-950  backdrop-blur-lg shadow-lg">
              <TableRow className="border-gray-700/50 hover:bg-gray-800/30">
                <TableHead className="w-36 pl-10 text-gray-200 font-semibold ">Company</TableHead>
              ...
              {companies?.map((company, index) => (
                ...
                >
                  ...
                  <TableCell className="text-gray-200 font-bold whitespace-normal break-words max-w-[7rem]">
                    <TruncatedCell text={company.package} limit={20} />
                  </TableCell>
                  <TableCell className="text-gray-300 font-medium whitespace-normal break-words max-w-[7rem]">
                    {company.deadline ? formatDate(company.deadline) : "N/A"}
                  </TableCell>
                  <TableCell >
